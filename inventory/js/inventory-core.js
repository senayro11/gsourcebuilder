// ============================================================
//  INVENTORY-CORE.JS — Self-contained DB engine + Auth + UI for Inventory
//  Own copy so this system reads/debugs independently of the root
//  app and the other systems -- mirrors the hr/ module's own
//  hr-core.js. Always used one folder deep (inventory/inventory.html), so links
...
// ============================================================
//  DB — GitHub API TXT database engine, own copy pointed at inventory/db
// ============================================================
const DB = (() => {
  ...
  // own default (inventory/db) always qualifies, so a Sync.flush() running
  ...
  // (inventory/db) -- used to reach tables shared at the root (productsDB,
...
const UI = (() => {
  ...
  // Own theme key (inventory_theme) -- independent from the root app's
  ...
  function toggleTheme(isDark) {
    ...
    localStorage.setItem('inventory_theme', isDark ? 'dark' : 'light');
  }
