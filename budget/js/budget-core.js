// ============================================================
//  BUDGET-CORE.JS — Self-contained DB engine + Auth + UI for Budget
//  Own copy so this system reads/debugs independently of the root
//  app and the other systems -- mirrors the hr/ module's own
//  hr-core.js. Always used one folder deep (budget/budget.html), so links
...
// ============================================================
//  DB — GitHub API TXT database engine, own copy pointed at budget/db
// ============================================================
const DB = (() => {
  ...
  // own default (budget/db) always qualifies, so a Sync.flush() running
  ...
  // (budget/db) -- used to reach tables shared at the root (productsDB,
...
const UI = (() => {
  ...
  // Own theme key (budget_theme) -- independent from the root app's
