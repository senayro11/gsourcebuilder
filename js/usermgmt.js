...
//  I-include sa bawat system page. Gumagamit ng shared modal.
// ============================================================
  ...
  <div class="modal modal-lg" style="max-width:860px">
    ...
    <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap">
      <div class="search-bar" style="flex:1;min-width:200px">
        <span>🔍</span>
        <input type="text" id="um-search" placeholder="Hanapin user..." oninput="UserMgmt.filter()">
      </div>
      ...
      <button class="btn btn-primary btn-sm" onclick="UserMgmt.openAdd()">+ Bagong User</button>
    </div>
    <!-- Table -->
...
<div class="modal-backdrop" id="um-form-backdrop">
  <div class="modal" style="max-width:480px">
    <div class="modal-header">
      <span class="modal-title" id="um-form-title">+ Bagong User</span>
      <button class="modal-close" onclick="UI.closeModal('um-form-backdrop')">✕</button>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Username *</label><input type="text" id="umf-username" placeholder="lowercase, walang spaces"></div>
    ...
    <div class="form-group">
      ...
      <div style="font-size:11px;color:var(--text3);margin-top:4px">ℹ️ Ang user na ito ay mag-a-access ng <strong id="umf-system-name"></strong> system lang.</div>
    </div>
    <div class="form-row">
      <div class="form-group">
    ...
    <div id="umf-pw-note" style="display:none;font-size:11px;color:var(--text3);margin:-8px 0 12px">ℹ️ Iwanang blangko ang password kung ayaw mong palitan.</div>
    <div class="form-group">
      <label>Status</label>
    ...
    <div class="modal-footer">
      <button class="btn btn-ghost" onclick="UI.closeModal('um-form-backdrop')">Kanselahin</button>
      <button class="btn btn-primary" onclick="UserMgmt.save()">I-save</button>
    </div>
  </div>
</div>`);
  }
  ...
  function renderTable(rows, page) {
    ...
    </tr>`).join('') || '<tr><td colspan="5" style="text-align:center;color:var(--text3)">Walang users</td></tr>';
  ...
  function openAdd() {
    ...
    document.getElementById('um-form-title').textContent='+ Bagong User';
  ...
  function openEdit(username) {
    ...
    document.getElementById('um-form-title').textContent='✏️ I-edit ang User';
  ...
  async function save() {
    ...
    if (!username||!fullName) { UI.toast('Punan ang required fields','warning'); return; }
    if (!editUsername && !pw) { UI.toast('Kailangan ng password para sa bagong user','warning'); return; }
    if (pw && pw !== pw2)  { UI.toast('Hindi match ang password!','error'); return; }
    if (pw && pw.length<6) { UI.toast('Min 6 characters ang password','warning'); return; }
    ...
    try {
      ...
      if (editUsername) {
        ...
        UI.toast('Na-update ang user ✅','success');
      } else {
        if (allAcc.find(u=>u.username===username)) {
          UI.toast('May existing user na na may ganitong username!','error');
        ...
        UI.toast('Naidagdag ang user ✅','success');
      }
  ...
  async function toggleStatus(username) {
    ...
    UI.confirm(`${newStatus==='suspended'?'I-suspend':'I-activate'} ang account ni ${u.full_name}?`, async()=>{
      UI.loading(true);
      try {
        await DB.update('accountsDB',r=>r.username===username,{status:newStatus});
        UI.toast(`Account ${newStatus} na ✅`,'success');
        allUsers=(await DB.read('accountsDB',true)).filter(u=>u.assigned_system===currentSystem);
        renderStats(); filter();
      } catch(e){UI.toast('Error: '+e.message,'error');}
      UI.loading(false);
  ...
  async function deleteUser(username) {
    UI.confirm(`I-DELETE ang account ni ${username}? Hindi ito mababawi.`, async()=>{
      UI.loading(true);
      try {
        await DB.remove('accountsDB',r=>r.username===username&&r.assigned_system===currentSystem);
        UI.toast('Na-delete ang user','success');
        allUsers=(await DB.read('accountsDB',true)).filter(u=>u.assigned_system===currentSystem);
        renderStats(); filter();
      } catch(e){UI.toast('Error: '+e.message,'error');}
      UI.loading(false);
...
// Global function para sa sidebar link
