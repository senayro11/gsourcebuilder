// ============================================================
//  PM-CORE.JS — Self-contained DB engine + Auth + UI for Project Management
//  Own copy so this system reads/debugs independently of the root
//  app and the other systems -- mirrors the hr/ module's own
//  hr-core.js. Always used one folder deep (pm/pm.html), so links
...
// ============================================================
//  DB — GitHub API TXT database engine, own copy pointed at pm/db
// ============================================================
const DB = (() => {
  ...
  // own default (pm/db) always qualifies, so a Sync.flush() running
  ...
  // (pm/db) -- used to reach tables shared at the root (productsDB,
...
const UI = (() => {
  ...
  // Own theme key (pm_theme) -- independent from the root app's
